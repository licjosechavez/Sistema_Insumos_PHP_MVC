  ...
  if (rows > 0) {
    ...
    $.ajax({
      ...
      if (response != 0) {
        var info = JSON.parse(response);
        //console.log(info);
        generarPDF(info.codcliente,info.nofactura, info.id_u);
        location.reload();
      }else {
        console.log('no hay dato');
      }
...
$('.view_factura').click(function(e) {
  ...
  var idusuario = $(this).attr('id_u');

  generarPDF(codCliente,noFactura, idusuario);
});
...
function generarPDF(cliente,factura,id_u) {
  url = 'factura/generaFactura.php?cl='+cliente+'&f='+factura+'&id_u='+id_u;
